...
jQuery.SelectBox = function(selectobj, options) {
  ...
  opt.loopnoStep = opt.loopnoStep || true; // to remove the step in list moves loop
  ...
  //container click in safari/opera
  var containerClickDetected = false;

  init(opt);
//
if ((!($select.is(':disabled')))&&($select.children('option').length!=0)) {
  ...
  .blur(function() {
    if ($container.is(':visible') && hasfocus > 0 ) {
      ...
      if(jQuery.browser.msie){ // check for safari too - workaround for webkit
        if ($(document.activeElement).attr('id').indexOf('_container') == -1) {
          hideMe();
        }
        else {
          $input.focus();
        ...
        if (!containerClickDetected) {
          hideMe();
        }
        else {
          $input.focus();
          containerClickDetected = false;
        }
      }
      else {
        hideMe();
    ...
    $input.keyup(function(event) {
      switch(event.keyCode) {
        ...
        case 27: //escape
          break;
        default:
          initSelectOptions($input.val(), false);
          $container.show();
          break;
      }
    });
...
//
  ...
  function initSelectOptions(filter, firstInit)
  {
    if (prevFilter === filter)
      return;
    prevFilter = filter;
    $('ul', $container).remove();
    $container.append(getSelectOptions($input.attr('id'), filter, firstInit));
  }

  function init(options) {
    initSelectOptions('', true);
    $container.hide();
//    $container.append(getSelectOptions($input.attr('id'))).hide();
    var width = $input.width();
    if($container.height() > opt.maxHeight){
      $container.width(width+21);//parseInt(width)+parseInt($input.css('paddingRight'))+parseInt($input.css('paddingLeft'))
      $container.height(opt.maxHeight);
    } else  $container.width(width+21);
//
    if ($select.is(':disabled')) $input.wrap('<span class="'+options.inputClass+'-left-corn fs-disabled" style="width:'+width+'px"><span class="'+options.inputClass+'-right-corn" style="width:'+width+'px"></span></span>')
    else $input.wrap('<span class="'+options.inputClass+'-left-corn" style="width:'+width+'px"><span class="'+options.inputClass+'-right-corn" style="width:'+width+'px"></span></span>')
  }
  ...
  function setupInput(options) {
    ...
    if(opt.inputType == "span"){
      ...
      $input.width(width);
      $input.html($select.children('option:selected').html());
    } else {
  ...
  function scroll(list, active) {
    ...
    if(opt.inputType == 'span') $input.html($(li).html());
      ...
      $input.val($(li).html().replace(tags, ''));
    }
    initSelectOptions('', true);
    $container.hide();
    return true;
  ...
  function getSelectOptions(parentid, filter, firstInit) {
    var select_options = new Array();
    var ul = document.createElement('ul');
    var regexp = new RegExp(pregQuote(filter), 'gi');
    ...
    if(select_options.length == 0) {
      ...
      if (filter !== '')
      {
        if (!title.match(regexp)) return;
        title = title.replace(regexp, '<b>$1</b>');
      }

      li.setAttribute('id', parentid + '_' + $(this).val());
      li.innerHTML = title;
      if (firstInit && $(this).is(':selected')) {
        $input.val($(this).html());
        $(li).addClass(opt.currentClass);
      }
      ul.appendChild(li);
