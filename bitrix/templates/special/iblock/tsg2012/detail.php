	...
	if ($arIBlockElement = GetIBlockElement($ID)):
		...
		?>
			<table cellpadding="5" cellspacing="0" border="0" width="100%">
				...
				<tr>
					<td>
						<?  if(strlen($arIBlockElement["PROPERTIES"]["dogovor1"]["VALUE"])>0 OR strlen($arIBlockElement["PROPERTIES"]["dohody"]["VALUE"])>0 OR strlen($arIBlockElement["PROPERTIES"]["deklaraziya"]["VALUE"])>0 OR strlen($arIBlockElement["PROPERTIES"]["perechen_uslug"]["VALUE"])>0 OR strlen($arIBlockElement["PROPERTIES"]["god_otchet"]["VALUE"])>0    OR strlen($arIBlockElement["PROPERTIES"]["dopolnenie"]["VALUE"])>0) :echo "<hr><font class=smalltext><strong>Список прилагаемых документов:</strong></font><br><br>"; ?>
                                                        ...
                                                        <?  if(strlen($arIBlockElement["PROPERTIES"]["dogovor1"]["VALUE"])>0): $DOCFILE1 = CFile::GetPath($arIBlockElement["PROPERTIES"]["dogovor1"]["VALUE"]);$date1 = $arIBlockElement['PROPERTIES']['date1']['VALUE'];echo "<tr><td><a href=$DOCFILE1><img src=/images/download.gif width=14 height=14 border=0 alt=\"Загрузить документ\" hspace=3 align=absmiddle>Договор на обслуживание МКД</a></td><td>$date1</td></tr>";?><?endif?>
                                                        ...
                                                        <?  if(strlen($arIBlockElement["PROPERTIES"]["god_otchet"]["VALUE"])>0): $DOCFILE7 = CFile::GetPath($arIBlockElement["PROPERTIES"]["god_otchet"]["VALUE"]);$date7 = $arIBlockElement['PROPERTIES']['date7']['VALUE'];echo "<tr><td><a href=$DOCFILE7><img src=/images/download.gif width=14 height=14 border=0 alt=\"Загрузить документ\" hspace=3 align=absmiddle>Годовой отчет выполнения работ по содержанию МКД</a></td><td>$date7</td></tr>";?><?endif?>			
                                                        <?  if(strlen($arIBlockElement["PROPERTIES"]["dopolnenie"]["VALUE"])>0): $DOCFILE8 = CFile::GetPath($arIBlockElement["PROPERTIES"]["dopolnenie"]["VALUE"]);$date8 = $arIBlockElement['PROPERTIES']['date8']['VALUE'];echo "<tr><td><a href=$DOCFILE8><img src=/images/download.gif width=14 height=14 border=0 alt=\"Загрузить документ\" hspace=3 align=absmiddle>Дополнения</a></td><td>$date8</td></tr>";?><?endif?>
                                                        </tbody>
                                                        </table>
