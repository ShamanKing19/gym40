...
<?
	...
	<table cellpadding="0" cellspacing="0" border="0" width="100%">
		...
		while($obItem = $items->GetNextElement()):
			...
			<tr>
				<td height="2"><?  if(strlen($arIBlockElement["PROPERTIES"]["docfile1"]["VALUE"])>0): $DOCFILE1 = CFile::GetPath($arIBlockElement["PROPERTIES"]["docfile1"]["VALUE"]); $DOCNAME1 = $arIBlockElement["PROPERTIES"]["docname1"]["VALUE"]; if ($DOCNAME1 != '') { $DOWNLOAD1 = $DOCNAME1;} else {$DOWNLOAD1 = "Скачать весь документ"; }echo "<a href='https://docs.google.com/viewerng/viewer?url=http://admgusev.ru/$DOCFILE1?AWSAccessKeyId%3DAKIAIKCGSVAFKYXP4VSQ%26Expires%3D1386830770%26Signature%3DHduBfc9fB6vwUGGL4SkGSCgE8cY%253D' target='_blank' onclick='newMyWindow(this.href); return false;'><img src=/images/download.gif width=14 height=14 border=0 alt=\"Загрузить документ\" hspace=3 align=absmiddle>$DOWNLOAD1</a>";?><br><?endif?>
					<?if(strlen($arIBlockElement["PROPERTIES"]["view"]["VALUE_ENUM"][0])>0){?>
						<div url-show="http://docs.google.com/viewer?url=http://admgusev.ru/<?=$DOCFILE1?>&amp;embedded=true" class="show_docs" onclick="showDocs(this); return false;">
							<img src=/images/download.gif width=14 height=14 border=0 alt="Загрузить документ" hspace=3 align=absmiddle>
							<a href="javascript:void(0)" class="docs">Просмотреть</a>
							<div class="file_block" style="height: 0px;">
								<iframe src="" height="980" width="650"></iframe>
							</div>
						</div>
					<?}?>
				</td>
	...
	if ($items->SelectedRowsCount() > 0):
		...
		?>
<form action="<? echo $APPLICATION->GetCurPage(); ?>" method="GET" id="nav_docs">
