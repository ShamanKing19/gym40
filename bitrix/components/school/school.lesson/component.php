...
if (is_numeric($arParams["ELEMENT_ID"])) {
	...
	if (count($arElementParams)==4) {
		...
		if ($arLesson) {
			...
			$format = $DB->DateFormatToPHP(CLang::GetDateFormat('FULL','s1'));
			$startDate = date($format,strtotime($lesson['time_start']=='00:00:00'?$date.' '.$lesson['lesson_start']:$date.' '.$lesson['time_start']));
			$endDate = date($format,strtotime($lesson['time_end']=='00:00:00'?$date.' '.$lesson['lesson_end']:$date.' '.$lesson['time_end']));
			$arFields = array(
				'NAME' => $arSubject['NAME'],
...
if($ELEMENT_ID>0 && $IBLOCK_ID > 0)
{
	...
	if($this->StartResultCache(false, array($arParams, $arUser["ID"])))
	{
		...
		$arMarktypes = MCSchool::GetMarkTypes();
		...
		foreach($arResult["PROPERTIES"]["MARKS"]["VALUE"] as $mv)
		{
			$MARKS[$mv["USER"]][$mv["TYPE"]] = array(
				"VALUE" => $mv["MARK"],
				"COLOR" => MCSchool::GetMarkColor($mv["MARK"]),
				);
		}
		...
		if($_REQUEST["mode"] == "ajax")
		{
			...
			switch($_REQUEST["action"])
			{
				...
				case "setMark":
					...
					echo MCSchool::GetMarkColor($mark);
					break;
