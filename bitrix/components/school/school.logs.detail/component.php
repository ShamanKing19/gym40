...
if ($this->StartResultCache(false, array($USER->GetID(), $arrFilter))) {
	...
	$arMarkTypes = Array();
	$arResult["MARK_TYPES"] = MCSchool::GetMarkTypes();
	foreach ($arResult["MARK_TYPES"] as $type => $val)
		$arMarkTypes[$type] = 1;
	$arAllMarks = MCSchool::GetMarks();
	$arResult["MARK_COLORS"] = Array();
	foreach ($arAllMarks as $mark)
		$arResult["MARK_COLORS"][$mark] = MCSchool::GetMarkColor($mark);
	...
	if ($scheduleRange['FIRST_WEEK'] && $scheduleRange['LAST_WEEK']) {
		...
		do {
			...
			$arPeriod = Array(
				"TITLE" => MCSchool::GetDiaryPeriod($firstDate, $lastDate),
	...
	$arResult["WEEK_PERIOD_TITLE"] = MCSchool::GetDiaryPeriod($ViewingWeekStart, $ViewingWeekEnd);
