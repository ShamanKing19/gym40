...
require_once($work_path.'/lang/'.LANGUAGE_ID.'/copy_week.php');
...
$ar=MCWeek::GetList(array('WEEK_START'=>date('Y-m-d',$_POST['week']),'CLASS'=>$_POST['empl'], 'SITE_ID'=>$siteID));
...
<div id="select_reload">
	<select style="display:none;" id="add_sel_period_type" name="add_sel_period_type" class="add_select">
		...
		?>
			...
			if (empty($dat_str)) {
				$dat_str = GetMessage('T_NO_SCHEDULE');
			}
		?>
	</select>
	<?echo $dat_str?>
...
<?
$week_selector = $work_path.'/../actions/week_selector.php' ;
