...
if ($arParams['EDIT_GROUP']) {
	if (!is_array($arParams['EDIT_GROUP']))
		$arParams['EDIT_GROUP'] = array($arParams['EDIT_GROUP']);
	foreach ($arParams['EDIT_GROUP'] as $key=>$value)
		$arParams['EDIT_GROUP'][$key] = intval($value);
}
else {
	$arParams['EDIT_GROUP'] = array(1);
}
function CanEdit($UserGroups, $EditGroup) {
	$canConfirm = array_intersect($UserGroups,$EditGroup);
	return !empty($canConfirm);
}
...
$pathStart					= substr(dirname($tmp_str), strpos($tmp_str, '/bitrix/'));
...
?>
	...
	function add_period(les, par_arr) {
		...
		jsAjaxUtil.ShowLocalWaitWindow(TID, obContainer, true);
		jsAjax.AddAction(TID, fvoid);
		jsAjax.Post(TID, '<?=$add_period_path?>', {'period_type':period_type, 'p_start':p_start, 'p_end':p_end, 'cabinet':cabinet, 'rtemplate_id':rtemplate_id, 'lesson':lesson, 'parms':par_arr, 'teacher':teacher, 'periods':periods, 'del_empl':del_empl, 'del_cab':del_cab, 'del_period':del_period, 'del_serv':del_serv, 'del_templ':del_templ, 'del_per':del_per, 'cancl':cancl});
	}
	function show_update_period(week, day, clas, empl, cabinet, period, service, tp, templ, p_num,t_start,t_end,les,cancl) {
	...
	function week_copy() {
		...
		jsAjaxUtil.ShowLocalWaitWindow(TID, obContainer, true);
		jsAjax.AddAction(TID, fvoid);
		jsAjax.Post(TID, '<?=$w_copy?>', {'class':clas, 'copy_from':cur_dat, 'copy_start':edit_dat, 'copy_count':cnt, 'skip_week':ch});
	}
